...
const InputForm: React.FC<InputFormProps> = ({ onCalculate, method }) => {
  ...
  const handleReset = () => {
    const initialTableData = Array.from({ length: rows }, () => Array(cols).fill(0));
    const initialWeights = Array(cols).fill(0);
    const initialTypes = Array(cols).fill("benefit");
